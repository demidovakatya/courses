...
There are 174 different event types due to similar values like these:
...
events[events=="ASTRONOMICAL LOW TIDE"] <- "ASTRONOMICAL LOW TIDE"
events[events=="AVALANCHE"] <- "AVALANCHE"
events[events=="BLIZZARD"] <- "BLIZZARD"
events[events=="COASTAL EROSION"] <- "COASTAL FLOOD"
events[events=="COASTAL FLOOD"] <- "COASTAL FLOOD"
events[events=="COASTAL FLOODING EROSION"] <- "COASTAL FLOOD"
events[events=="COASTAL FLOODING"] <- "COASTAL FLOOD"
events[events=="COASTAL STORM"] <- "COASTAL FLOOD"
events[events=="COASTALFLOODING EROSION"] <- "COASTAL FLOOD"
events[events=="COASTALSTORM"] <- "COASTAL FLOOD"
events[events=="COLD TEMPERATURE"] <- "COLD/WIND CHILL"
events[events=="COLD WEATHER"] <- "COLD/WIND CHILL"
events[events=="COLD WIND CHILL"] <- "COLD/WIND CHILL"
events[events=="COLD"] <- "COLD/WIND CHILL"
events[events=="DAMAGING FREEZE"] <- "FROST/FREEZE"
events[events=="DENSE FOG"] <- "DENSE FOG"
events[events=="DOWNBURST"] <- "STRONG WIND"
events[events=="DROUGHT"] <- "DROUGHT"
events[events=="DRY MICROBURST"] <- "STRONG WIND"
events[events=="DUST DEVIL"] <- "DUST DEVIL"
events[events=="DUST STORM"] <- "DUST STORM"
events[events=="EROSION CSTL FLOOD"] <- "COASTAL FLOOD"
events[events=="EXCESSIVE HEAT"] <- "EXCESSIVE HEAT"
events[events=="EXTREME COLD WIND CHILL"] <- "EXTREME COLD/WIND CHILL"
events[events=="EXTREME COLD"] <- "EXTREME COLD/WIND CHILL"
events[events=="EXTREME WINDCHILL"] <- "EXTREME COLD/WIND CHILL"
events[events=="FLASH FLOOD FLOOD"] <- "FLOOD/FLASH FLOOD"
events[events=="FLASH FLOOD"] <- "FLOOD/FLASH FLOOD"
events[events=="FLOOD FLASH FLOOD"] <- "FLOOD/FLASH FLOOD"
events[events=="FLOOD"] <- "FLOOD/FLASH FLOOD"
events[events=="FREEZING FOG"] <- "FREEZING FOG"
events[events=="FROST FREEZE"] <- "FROST/FREEZE"
events[events=="FROST"] <- "FROST/FREEZE"
events[events=="FUNNEL CLOUD"] <- "FUNNEL CLOUD"
events[events=="HAIL"] <- "HAIL"
events[events=="HEAT"] <- "HEAT"
events[events=="HEAVY RAIN HIGH SURF"] <- "HEAVY RAIN"
events[events=="HEAVY RAIN"] <- "HEAVY RAIN"
events[events=="HEAVY SNOW SHOWER"] <- "HEAVY SNOW"
events[events=="HEAVY SNOW"] <- "HEAVY SNOW"
...
events[events=="HIGH SURF"] <- "HIGH SURF"
events[events=="HIGH WIND"] <- "HIGH WIND"
...
events[events=="LIGHTNING"] <- "LIGHTNING"
events[events=="MARINE HAIL"] <- "MARINE HAIL"
events[events=="MARINE HIGH WIND"] <- "MARINE HIGH WIND"
events[events=="MARINE STRONG WIND"] <- "MARINE STRONG WIND"
events[events=="MARINE THUNDERSTORM WIND"] <- "MARINE THUNDERSTORM WIND"
events[events=="RIP CURRENT"] <- "RIP CURRENT"
events[events=="RIP CURRENTS"] <- "RIP CURRENT"
events[events=="SEICHE"] <- "SEICHE"
events[events=="SMALL HAIL"] <- "HAIL"
events[events=="STRONG WIND"] <- "STRONG WIND"
events[events=="STRONG WINDS"] <- "STRONG WIND"
events[events=="THUNDERSTORM WIND"] <- "THUNDERSTORM WIND"
events[events=="TORNADO"] <- "TORNADO"
events[events=="TROPICAL DEPRESSION"] <- "TROPICAL DEPRESSION"
events[events=="TROPICAL STORM"] <- "TROPICAL STORM"
...
events[events=="TSUNAMI"] <- "TSUNAMI"
events[events=="TYPHOON"] <- "HURRICANE/TYPHOON"
events[events=="VOLCANIC ASH"] <- "VOLCANIC ASH"
events[events=="WATERSPOUT"] <- "WATERSPOUT"
events[events=="WILD FOREST FIRE"] <- "WILDFIRE"
events[events=="WILDFIRE"] <- "WILDFIRE"
events[events=="WINTER STORM"] <- "WINTER STORM"
events[events=="WINTER WEATHER MIX"] <- "WINTER WEATHER"
events[events=="WINTER WEATHER"] <- "WINTER WEATHER"
...
events[events=="LANDSLIDE"] <- "LANDSLIDE"
...
events[events=="MUDSLIDE"] <- "MUDSLIDE"
...
ggplot(aes(evtype, property.damage/10^9, fill=evtype), data=property.damage.events) + geom_bar(stat="identity") + xlab("Event type")+ ylab("Property damage cost ($ billions)") + ggtitle("15 events causing most property damage") + theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position="none") + coord_flip()
```
...
ggplot(aes(evtype, crop.damage/10^9, fill=evtype), data=crop.damage.events) + geom_bar(stat="identity") + xlab("Event type")+ ylab("Crop damage cost ($ billions)") + ggtitle("15 events causing most crop damage") + theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position="none") + coord_flip()
