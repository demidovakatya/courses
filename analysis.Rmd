...
```{r, cache=TRUE}
storm <- raw.storm %>% select(one_of(c("BGN_DATE", "BGN_TIME", "END_DATE", "END_TIME", "STATE", "EVTYPE", "FATALITIES", "INJURIES", "PROPDMG", "PROPDMGEXP", "CROPDMG", "CROPDMGEXP")))
...
```{r, cache=TRUE}
storm <- storm %>% mutate(property.damage=propdmg * 10 ^ propdmgexp)
storm <- storm %>% mutate(property.damage=cropdmg * 10 ^ cropdmgexp)
```
```{r, cache=TRUE}
```
## Results
