...
With p-value of ``r t.test(mpg ~ am, data = mtcars)$p.value``, we can reject the null hypothesis and see that this difference is significant.
...
ggplot(aes(mpg), data = mtcars) + geom_histogram() + xlab("Miles per gallon")
ggplot(aes(cyl), data = mtcars) + geom_bar() + xlab("Number of cylinders")
ggplot(aes(hp), data = mtcars) + geom_histogram() + scale_x_continuous(labels = seq(0,350,50), breaks = seq(0,350,50)) + xlab("Gross horsepower")
```
