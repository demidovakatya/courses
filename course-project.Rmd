...
In this analysis, we will use the `mtcars` data set. This data was extracted from the 1974 *Motor Trend* US magazine. It comprises fuel consumption and 10 aspects of automobile design and performance for 32 automobiles (1973–74 models). First, we will load the data and perform basic exploration:
...
Next, we will explore correlations between all variables in the `mtcars` dataset (Appendix - Figure: Correlations between variables). 
...
With p-value of `r t.test(mpg ~ am, data = mtcars)$p.value`, we can reject the null hypothesis see that this difference is significant.
