...
# point
# 1.
# Выберете из перечисленных два варианта, которые являются корректными действиями при проверке гипотез
#
...
# point
# 2.
# Проверив гипотезу, можно сделать статистический вывод о том, что
#
...
# point
# 3.
# Какие два из приведенных ниже свойств входят в БСХС (Большой Список Хороших Свойств)?
#
...
# point
# 4.
# Понятие несмещенности оценок означает, что
#
# E(\hat{\beta}_j | X) = 0
#
# E(\hat{\beta}_j | X) = 1
...
# point
# 5.
# Что может означать \bar{X} ± t_{\alpha/2}(n-1) \frac{\sigma}{\sqrt{n}}?
#
...
# point
# 6.
# По 1 000 наблюдениям была оценена следующая зависимость стоимости квадратного метра квартиры \it{price}_i ($) от времени до ближайшего общественного транспорта \it{time}_i (минуты) и расстояния до центра города \it{distance}_i (км):
#
...
# point
# 7.
# По 52 наблюдениям оценена модель зависимости заработной платы \it{wage}_i ($) от длительности обучения {schooling}_i (годы). Оценённая модель имеет вид:
#
...
# point
# 8.
# Оценённая по 30 наблюдениям модель имеет вид:
#
...
# point
# 9.
# При проверке гипотезы о значимости коэффициента альтернативная гипотеза имеет следующий вид:
#
# \beta \not= 0
#
# \hat{\beta} \not= 0
...
# point
# 10.
# Симметричные 90%-ые доверительные интервалы для коэффициентов \beta_2 и \beta_3 в регрессии y_i = \beta_1 + \beta_2x_i + \beta_3z_i + \varepsilon_i имеют вид \beta_2 \in (-0.15,0.75) и \beta_3 \in (0.5,1.5). Найдите \hat{\beta}_2.
#
# 1
# point
# 11.
# Используя команду R pchisq(..., df=...), посчитайте вероятность P(Z < 9), если Z имеет \chi^2 распределение с 10 степенями свободы. Округляйте до двух знаков после запятой.
#
# 1
# point
# 12.
# Регрессионная модель, которая имеет вид y_i = \beta_1 + \beta_2x_i + \varepsilon_i, в R оценивается, например, с помощью команды regression <- glm(y ~ x, data). C помощью какой команды в R можно посмотреть на стандартные ошибки оценок коэффициентов в этой регрессии и их t-статистики?
#
# str(regression)
#
# coef(regression)
...
#
# Для выполнения данного задания необходимо подгрузить данные по бриллиантам (diamonds) из пакета ggplot2 и поместить их в табличку data. Это можно сделать командами: library("ggplot2") data <- diamonds

table(data$cut)['Ideal']
# Ideal
# 21551
...
# point
# 17.
# В модели {price}_i = \beta_1 + \beta_2 {carat}_i + {\varepsilon}_i на уровне значимости 5% проверьте гипотезу о том, что количество карат не оказывает влияния на стоимость бриллиантов.
#
# Для выполнения данного задания необходимо подгрузить данные по бриллиантам (diamonds) из пакета ggplot2 и поместить их в табличку data. Это можно сделать командами: library("ggplot2") data <- diamonds
#
#
...
# point
# 18.
# В модели {price}_i = \beta_1 + \beta_2 {carat}_i + \beta_3 x_i + \beta_4 y_i + \beta_5 z_i + {\varepsilon}_i при проверке гипотезы о влиянии ширины бриллианта (y) на его стоимость на уровне значимости 1% чему равно P-значение?
#
# Для выполнения данного задания необходимо подгрузить данные по бриллиантам (diamonds) из пакета ggplot2 и поместить их в табличку data. Это можно сделать командами: library("ggplot2") data <- diamonds
#
#
...
# point
# 19.
# Сначала оценили модель {price}_i = \beta_1 + \beta_2{carat}_i + {\varepsilon}_i, потом оценили другую модель {price}_i = \beta_1 + \beta_2{carat}_i + \beta_3y_i + {\varepsilon}_i. Изменилась ли интерпретация коэффициента при количестве карат?
#
# Не изменилась
